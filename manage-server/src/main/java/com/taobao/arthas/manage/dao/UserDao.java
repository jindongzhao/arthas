...
public interface UserDao extends JpaRepository<User, Long> {
	...
	/**
	 * Query creation this translates into the following query: select u from User u
	 * where u.idCard = ?1
	 */
	User findByIdCard(String idCard);

	/**
	 * Native Queries The @Query annotation allows for running native queries by
	 * setting the nativeQuery flag to true
	 * 
	 * @param idCard
	 * @return
	 */
	...
	/**
	 * Declare native count queries for pagination at the query method by
	 * using @Query
	 * 
	 * @param lastName
	 * @param pageable
